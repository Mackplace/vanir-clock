var clockGroupThursday, fields, formatHour, formatMinute, formatSecond, height, offSetX, offSetY, pi, renderThursday, scaleHours, scaleSecsMins, vis, width;
...
clockGroupThursday = vis.append("svg:g")
...
for (i = 0; i < thusrday.length; i++) {

    clockGroupThursday.append("svg:circle")
        ...
        .attr("stroke-width", 2);
    for (j = 0; j < thusrday[i].length; j++) {



        console.log(thusrday[i][j].intervalo);

        var arc = d3.svg.arc()
            .innerRadius(0)
            .outerRadius((radius+120)*(1-(i*0.1)))
            .startAngle(clockToRad(hourToDec(thusrday[i][j].intervalo)[0], -1))
            .endAngle(clockToRad(hourToDec(thusrday[i][j].intervalo)[1], 1));
        arcs[i][j] = arc;
        clockGroupThursday.append('path')
            .attr('d', arcs[i][j])
            .attr("class", thusrday[i][j].Activity)
            .style('fill', colors[i])
            .style("stroke", colors[i])
            .attr("intervalo",thusrday[i][j].intervalo)
            ...
            .on('mouseover',function(d)
            {
                ...
                clockGroupThursday.selectAll('path')
                    .style("fill-opacity",.3)
                    .style("stroke-opacity", .3);


                clockGroupThursday.selectAll(z)
...
clockGroupThursday.append("svg:circle")
    .attr("r", (radius+120)*(1-((thusrday.length)*0.1)))
    .attr("fill", "white")
    .attr("class", "clock outercircle")
    .attr("stroke", "black")
    .attr("stroke-opacity", .3)
...
clockGroupThursday.selectAll('.minuteLabel')
...
renderThursday = function(data) {
    var hourArc, minuteArc, secondArc;
    clockGroupThursday.selectAll(".clockhand").remove();
    ...
    clockGroupThursday.selectAll(".clockhand")
        .data(data)
        .enter()
        .append("svg:path")
        .attr("d", function(d) {
...
setInterval(function() {
    var data;
    data = fields();
    return renderThursday(data);
}, 1000);

var w = 500,
    h = 500;

var text = clockGroupThursday.append("text")
    .attr("class", "title")
...
var text0 = clockGroupThursday.append("text")
    .attr("class", "title")
    .attr('transform', 'translate(-300,-90)')
    .attr("x", w - 204)
    .attr("y", 35)
...
var text1 = clockGroupThursday.append("text")
    .attr("class", "title")
    .attr('transform', 'translate(-300,-90)')
    .attr("x", w-140 )
    .attr("y", 95)
...
var text2 = clockGroupThursday.append("text")
    .attr("class", "title")
    .attr('transform', 'translate(-300,-90)')
    .attr("x", w-210 )
    .attr("y", 155)
...
var legend = clockGroupThursday.append('g')
    .attr("class", "legend")
    .attr("height", 100)
    .attr("width", 200)
...
for (i = 0; i < thusrday.length; i++) {
    console.log(thusrday[i][1].Activity);
    //Create colour squares
    legend.append("rect")
        .attr("class", thusrday[i][1].Activity)
        .attr("x", w +100)
        .attr("y", i*20)
        .attr("width", 10)
        .attr("height", 10)
