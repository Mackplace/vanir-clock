...
vis = d3.selectAll(".monday")
    .append("svg:svg")
    .attr("width", width)
    .attr("height", height);

clockGroupMonday = vis.append("svg:g")
...
for (i = 0; i < monday.length; i++) {
    ...
    clockGroupMonday.append("svg:circle")
        ...
        .attr("stroke-width", 2);
    for (j = 0; j < monday[i].length; j++) {



        console.log(monday[i][j].intervalo);

        var arc = d3.svg.arc()
            .innerRadius(0)
            .outerRadius((radius+120)*(1-(i*0.1)))
            .startAngle(clockToRad(hourToDec(monday[i][j].intervalo)[0], -1))
            .endAngle(clockToRad(hourToDec(monday[i][j].intervalo)[1], 1));
        arcs[i][j] = arc;
        clockGroupMonday.append('path')
            .attr('d', arcs[i][j])
            .attr("class", monday[i][j].Activity)
            .style('fill', colors[i])
...
clockGroupMonday.append("svg:circle")
    .attr("r", (radius+120)*(1-((monday.length)*0.1)))
    .attr("fill", "white")
    .attr("class", "clock outercircle")
    .attr("stroke", "black")
    .attr("stroke-opacity", .3)
...
renderMonday = function(data) {
    var hourArc, minuteArc, secondArc;
    clockGroupMonday.selectAll(".clockhand").remove();
    ...
    clockGroupMonday.selectAll(".clockhand")
        .data(data)
        .enter()
        .append("svg:path")
        .attr("d", function(d) {
...
var text0 = clockGroupMonday.append("text")
    .attr("class", "title")
    .attr('transform', 'translate(-300,-90)')
    .attr("x", w - 204)
    .attr("y", 35)
...
for (i = 0; i < monday.length; i++) {
    console.log(monday[i][1].Activity);
    //Create colour squares
    legend.append("rect")
        .attr("class", monday[i][1].Activity)
        .attr("x", w +100)
        .attr("y", i*20)
        .attr("width", 10)
        .attr("height", 10)
