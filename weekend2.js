var clockGroupWeekend, fields, formatHour, formatMinute, formatSecond, height, offSetX, offSetY, pi, renderWeekend, scaleHours, scaleSecsMins, vis, width;
...
vis = d3.selectAll(".weekend")
    .append("svg:svg")
    .attr("width", width)
    .attr("height", height);

clockGroupWeekend = vis.append("svg:g")
...
for (i = 0; i < weekeend.length; i++) {
    ...
    clockGroupWeekend.append("svg:circle")
    ...
    for (j = 0; j < weekeend[i].length; j++) {
        ...
        console.log(weekeend[i][j].intervalo);

        var arc = d3.svg.arc()
            .innerRadius(0)
            .outerRadius((radius+120)*(1-(i*0.1)))
            .startAngle(clockToRad(hourToDec(weekeend[i][j].intervalo)[0], -1))
            .endAngle(clockToRad(hourToDec(weekeend[i][j].intervalo)[1], 1));
        arcs[i][j] = arc;
        clockGroupWeekend.append('path')
            .attr('d', arcs[i][j])
            .attr("class", weekeend[i][j].Activity)
            .style('fill', colors[i])
            .style("stroke", colors[i])
            .attr("intervalo",weekeend[i][j].intervalo)
            ...
            .on('mouseover',function(d)
            {
                ...
                clockGroupWeekend.selectAll('path')
                    .style("fill-opacity",.3)
                    .style("stroke-opacity", .3);


                clockGroupWeekend.selectAll(z)
...
clockGroupWeekend.append("svg:circle")
    .attr("r", (radius+120)*(1-((weekeend.length)*0.1)))
    .attr("fill", "white")
    .attr("class", "clock outercircle")
    .attr("stroke", "black")
    .attr("stroke-opacity", .3)
...
clockGroupWeekend.selectAll('.minuteLabel')
...
renderWeekend = function(data) {
    var hourArc, minuteArc, secondArc;
    clockGroupWeekend.selectAll(".clockhand").remove();
    ...
    clockGroupWeekend.selectAll(".clockhand")
        .data(data)
        .enter()
        .append("svg:path")
        .attr("d", function(d) {
...
setInterval(function() {
    var data;
    data = fields();
    return renderWeekend(data);
}, 1000);

var w = 500,
    h = 500;

var text = clockGroupWeekend.append("text")
    .attr("class", "title")
...
var text0 = clockGroupWeekend.append("text")
    .attr("class", "title")
    .attr('transform', 'translate(-300,-90)')
    .attr("x", w - 204)
    .attr("y", 35)
...
var text1 = clockGroupWeekend.append("text")
    .attr("class", "title")
    .attr('transform', 'translate(-300,-90)')
    .attr("x", w-140 )
    .attr("y", 95)
...
var text2 = clockGroupWeekend.append("text")
    .attr("class", "title")
    .attr('transform', 'translate(-300,-90)')
    .attr("x", w-210 )
    .attr("y", 155)
...
var legend = clockGroupWeekend.append('g')
    .attr("class", "legend")
    .attr("height", 100)
    .attr("width", 200)
...
for (i = 0; i < weekeend.length; i++) {
    console.log(weekeend[i][1].Activity);
    //Create colour squares
    legend.append("rect")
        .attr("class", weekeend[i][1].Activity)
        .attr("x", w +100)
        .attr("y", i*20)
        .attr("width", 10)
        .attr("height", 10)
