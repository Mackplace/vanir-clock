var clockGroupWednesday, fields, formatHour, formatMinute, formatSecond, height, offSetX, offSetY, pi, renderWednesday, scaleHours, scaleSecsMins, vis, width;
...
vis = d3.selectAll(".wednesday")
    .append("svg:svg")
    .attr("width", width)
    .attr("height", height);

clockGroupWednesday = vis.append("svg:g")
...
for (i = 0; i < wednesday.length; i++) {
    ...
    clockGroupWednesday.append("svg:circle")
        ...
        .attr("stroke-width", 2);
    for (j = 0; j < wednesday[i].length; j++) {



        console.log(wednesday[i][j].intervalo);

        var arc = d3.svg.arc()
            .innerRadius(0)
            .outerRadius((radius+120)*(1-(i*0.1)))
            .startAngle(clockToRad(hourToDec(wednesday[i][j].intervalo)[0], -1))
            .endAngle(clockToRad(hourToDec(wednesday[i][j].intervalo)[1], 1));
        arcs[i][j] = arc;
        clockGroupWednesday.append('path')
            .attr('d', arcs[i][j])
            .attr("class", wednesday[i][j].Activity)
            .style('fill', colors[i])
            ...
            .on('mouseover',function(d)
            {
                ...
                clockGroupWednesday.selectAll('path')
                    .style("fill-opacity",.3)
                    .style("stroke-opacity", .3);


                clockGroupWednesday.selectAll(z)
...
clockGroupWednesday.append("svg:circle")
    .attr("r", (radius+120)*(1-((wednesday.length)*0.1)))
    .attr("fill", "white")
    .attr("class", "clock outercircle")
    .attr("stroke", "black")
    .attr("stroke-opacity", .3)
...
clockGroupWednesday.selectAll('.minuteLabel')
...
renderWednesday = function(data) {
    var hourArc, minuteArc, secondArc;
    clockGroupWednesday.selectAll(".clockhand").remove();
    ...
    clockGroupWednesday.selectAll(".clockhand")
        .data(data)
        .enter()
        .append("svg:path")
        .attr("d", function(d) {
...
setInterval(function() {
    var data;
    data = fields();
    return renderWednesday(data);
}, 1000);

var w = 500,
    h = 500;

var text = clockGroupWednesday.append("text")
    .attr("class", "title")
...
var text0 = clockGroupWednesday.append("text")
    .attr("class", "title")
    .attr('transform', 'translate(-300,-90)')
    .attr("x", w - 204)
    .attr("y", 35)
...
var text1 = clockGroupWednesday.append("text")
    .attr("class", "title")
    .attr('transform', 'translate(-300,-90)')
    .attr("x", w-140 )
    .attr("y", 95)
...
var text2 = clockGroupWednesday.append("text")
    .attr("class", "title")
    .attr('transform', 'translate(-300,-90)')
    .attr("x", w-210 )
    .attr("y", 155)
...
var legend = clockGroupWednesday.append('g')
    .attr("class", "legend")
    .attr("height", 100)
    .attr("width", 200)
...
for (i = 0; i < wednesday.length; i++) {
    console.log(wednesday[i][1].Activity);
    //Create colour squares
    legend.append("rect")
        .attr("class", wednesday[i][1].Activity)
        .attr("x", w +100)
        .attr("y", i*20)
        .attr("width", 10)
        .attr("height", 10)
