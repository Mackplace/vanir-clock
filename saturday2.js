 ...
 */
var clockGroupSaturday, fields, formatHour, formatMinute, formatSecond, height, offSetX, offSetY, pi, renderSaturday, scaleHours, scaleSecsMins, vis, width;
...
vis = d3.selectAll(".saturday")
    .append("svg:svg")
    .attr("width", width)
    .attr("height", height);

clockGroupSaturday = vis.append("svg:g")
...
for (i = 0; i < saturday.length; i++) {
    ...
    clockGroupSaturday.append("svg:circle")
        ...
        .attr("stroke-width", 2);
    for (j = 0; j < saturday[i].length; j++) {



        console.log(saturday[i][j].intervalo);

        var arc = d3.svg.arc()
            .innerRadius(0)
            .outerRadius((radius+120)*(1-(i*0.1)))
            .startAngle(clockToRad(hourToDec(saturday[i][j].intervalo)[0], -1))
            .endAngle(clockToRad(hourToDec(saturday[i][j].intervalo)[1], 1));
        arcs[i][j] = arc;
        clockGroupSaturday.append('path')
            .attr('d', arcs[i][j])
            .attr("class", saturday[i][j].Activity)
            .style('fill', colors[i])
            .style("stroke", colors[i])
            .attr("intervalo",saturday[i][j].intervalo)
            ...
            .on('mouseover',function(d)
            {
                ...
                clockGroupSaturday.selectAll('path')
                    .style("fill-opacity",.3)
                    .style("stroke-opacity", .3);


                clockGroupSaturday.selectAll(z)
...
clockGroupSaturday.append("svg:circle")
    .attr("r", (radius+120)*(1-((saturday.length)*0.1)))
    .attr("fill", "white")
    .attr("class", "clock outercircle")
    .attr("stroke", "black")
    .attr("stroke-opacity", .3)
...
clockGroupSaturday.selectAll('.minuteLabel')
...
renderSaturday = function(data) {
    var hourArc, minuteArc, secondArc;
    clockGroupSaturday.selectAll(".clockhand").remove();
    ...
    clockGroupSaturday.selectAll(".clockhand")
        .data(data)
        .enter()
        .append("svg:path")
        .attr("d", function(d) {
...
setInterval(function() {
    var data;
    data = fields();
    return renderSaturday(data);
}, 1000);

var w = 500,
    h = 500;

var text = clockGroupSaturday.append("text")
    .attr("class", "title")
...
var text0 = clockGroupSaturday.append("text")
    .attr("class", "title")
    .attr('transform', 'translate(-300,-90)')
    .attr("x", w - 204)
    .attr("y", 35)
...
var text1 = clockGroupSaturday.append("text")
    .attr("class", "title")
    .attr('transform', 'translate(-300,-90)')
    .attr("x", w-140 )
    .attr("y", 95)
...
var text2 = clockGroupSaturday.append("text")
    .attr("class", "title")
    .attr('transform', 'translate(-300,-90)')
    .attr("x", w-210 )
    .attr("y", 155)
...
var legend = clockGroupSaturday.append('g')
    .attr("class", "legend")
    .attr("height", 100)
    .attr("width", 200)
...
for (i = 0; i < saturday.length; i++) {
    console.log(saturday[i][1].Activity);
    //Create colour squares
    legend.append("rect")
        .attr("class", saturday[i][1].Activity)
        .attr("x", w +100)
        .attr("y", i*20)
        .attr("width", 10)
        .attr("height", 10)
